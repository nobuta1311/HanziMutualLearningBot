...
function command_carousel($userinfo){
    ...
    $actions_parameter=["BASE?mode=0","BASE?mode=1","BASE?mode=2","PRO?hanzi=NULL","MEAN?hanzi=NULL","RESULT","BASE?mode=6","BASE?mode=7","OTHERS","USERCONF","CHARCONF","BASE?mode=11"];	//12個
    for($i=0;$i<4;$i++) {
    // カルーセルに付与するボタンを作る
    	for($j=0;$j<3;$j++){
